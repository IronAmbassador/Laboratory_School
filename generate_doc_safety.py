...
public class BookingController extends BaseController<BookingService, Booking> {
    ...
    doc.add_heading('（1）功能描述：', level=4)
    doc.add_paragraph(
        '管理实验室的安全检查记录，包括检查日期、检查人员、检查项目、检查结果、整改情况等。'
        '支持安全检查记录的增删改查操作，以及检查结果的统计分析。'
    )
    
    doc.add_heading('（2）设计思路：', level=4)
    doc.add_paragraph(
        '在安全检查登记页面可以查看所有检查记录，按实验室、检查日期等条件筛选。'
        '新增检查记录时选择所属实验室，填写检查日期、检查人员、检查项目、检查结果等信息。'
        '检查结果包括：合格、基本合格、不合格。需要整改的记录可以记录整改情况。'
        '系统提供检查结果的统计功能，可以查看各实验室的安全状况。'
    )
    
    doc.add_heading('（3）关键技术：', level=4)
    doc.add_paragraph(
        '使用EasyExcel进行安全检查记录的导入导出，使用MyBatis Plus进行数据库操作，'
        '使用ECharts进行数据可视化统计分析。'
    )
    
    doc.add_heading('前端主要代码：', level=4)
    safety_check_api_code = '''// device/index.js - 安全检查登记API
import ApiClient from '../base-api'

const safetyCheckApi = new ApiClient('/admin/device')

// 获取检查记录总数
safetyCheckApi.getTotal = () => {
  return safetyCheckApi.get('total')
}

// 获取合格记录数
safetyCheckApi.getQualifiedCount = () => {
  return safetyCheckApi.get('getQualifiedCount')
}

// 获取不合格记录数
safetyCheckApi.getUnqualifiedCount = () => {
  return safetyCheckApi.get('getUnqualifiedCount')
}

export default safetyCheckApi'''
    doc.add_paragraph(safety_check_api_code)
...
public class DeviceController extends BaseController<DeviceService, Device> {
    ...
    @ApiOperation(value = "获取合格记录数")
    @GetMapping("getQualifiedCount")
    public Result<?> getQualifiedCount(){
        return Result.success(baseService.getQualifiedCount());
    }

    @ApiOperation(value = "获取不合格记录数")
    @GetMapping("getUnqualifiedCount")
    public Result<?> getUnqualifiedCount(){
        return Result.success(baseService.getUnqualifiedCount());
    }
}'''
    doc.add_paragraph(safety_check_controller_code)
    
    safety_check_entity_code = '''// Device.java - 安全检查记录实体类
@ApiModel(value = "安全检查记录")
@Data
...
public class Device extends BaseEntity implements Serializable {
    ...
    @ApiModelProperty(value = "检查结果: 0-不合格，1-合格，2-基本合格")
    ...
    doc.add_paragraph(safety_check_entity_code)
    
    doc.add_heading('（4）运行界面（截图）', level=4)
    doc.add_paragraph('图7 安全检查登记界面')
    doc.add_paragraph('图8 添加安全检查记录界面')
    doc.add_paragraph('【此处插入安全检查登记界面截图】')
    doc.add_paragraph('【此处插入添加安全检查记录界面截图】')
...
if __name__ == '__main__':
    output_file = create_safety_check_doc()
    print(f"技术开发文档生成完成！")
