...
# -*- coding: utf-8 -*-
"""
实验室管理系统 - 技术开发文档生成脚本（基于实际代码：安全检查登记）
"""
...
def create_doc_with_real_safety_check():
    """创建基于实际代码的安全检查登记文档"""
    doc = Document()
...
public class BookingController extends BaseController<BookingService, Booking> {
    ...
    # 2.4 安全检查登记（基于实际代码）
    doc.add_heading('2.4 安全检查登记', level=3)
    doc.add_heading('（1）功能描述：', level=4)
    doc.add_paragraph(
        '管理实验室的安全检查记录，包括检查项、检查日期、检查人员、检查状态等信息。'
        '支持按学期、实验室、检查状态等条件筛选查询。'
        '检查状态包括：合格、待整改、不合格等。'
        '支持安全检查记录的增删改查操作，以及批量导入导出功能。'
    )
    
    doc.add_heading('（2）设计思路：', level=4)
    doc.add_paragraph(
        '在安全检查登记页面可以查看所有检查记录，支持按学期、实验室、检查状态等条件筛选。'
        '新增检查记录时选择所属实验室和学期，填写检查项、检查日期、检查人员、检查状态等信息。'
        '支持批量删除和批量导出功能，方便管理员进行数据管理。'
        '系统提供统计功能，可以查看各实验室的安全检查情况。'
    )
    
    doc.add_heading('（3）关键技术：', level=4)
    doc.add_paragraph(
        '使用EasyExcel进行安全检查记录的导入导出，使用MyBatis Plus进行数据库操作，'
        '前端使用Element UI的表格和表单组件，后端使用Spring Boot RESTful API。'
    )
    
    doc.add_heading('前端主要代码：', level=4)
    safety_check_vue_code = '''// device/index.vue - 安全检查登记前端页面（核心代码）
<template>
  <div class="app-container">
    ...
    <!-- 数据表格 -->
    <el-table :data="data.list" stripe style="width: 100%; margin: 15px 0px">
      <el-table-column type="index" width="80" label="序号" />
      <el-table-column prop="labName" label="实验室" />
      <el-table-column prop="deviceName" label="检查项" />
      <el-table-column prop="deviceDate" label="检查日期" sortable />
      <el-table-column prop="guarantee" label="学期" sortable />
      <el-table-column prop="deviceFactory" label="检查人员" />
      <el-table-column prop="deviceStatus" label="检查状态">
        <template v-slot="scope">
          <el-tag v-for="item in deviceStatusOptions" :key="item.value"
                  :type="item.type" v-if="scope.row.deviceStatus == item.value">
            {{ item.text }}
          </el-tag>
        </template>
      </el-table-column>
    ...
    doc.add_paragraph(safety_check_vue_code)
    
    doc.add_heading('后端主要代码：', level=4)
    safety_check_controller_code = '''// DeviceController.java - 安全检查登记控制器
@Api(tags = "实验室设备管理（安全检查）")
@RestController
@RequestMapping("/admin/device")
public class DeviceController extends BaseController<DeviceService, Device> {

    @ApiOperation(value = "分页查询所有安全检查记录")
    @PostMapping("/page/{pIndex}/{pSize}")
    ...
    @ApiOperation(value = "获取检查记录总数")
    @GetMapping("total")
    public Result<?> getTotal(){
        return Result.success(baseService.getTotal());
    }

    @ApiOperation(value = "获取待整改记录数")
    @GetMapping("getRepairTotal")
    public Result<?> getRepairTotal(){
        return Result.success(baseService.getRepairTotal());
    }
}'''
    doc.add_paragraph(safety_check_controller_code)
    
    device_entity_code = '''// Device.java - 安全检查记录实体类
@ApiModel(value = "实验室设备（安全检查）")
@Data
@EqualsAndHashCode(callSuper = true)
public class Device extends BaseEntity implements Serializable {

    @ApiModelProperty(value = "设备名称（实际用作：检查项）")
    private String deviceName;

    @ApiModelProperty(value = "购买时间（实际用作：检查日期）")
    private Date deviceDate;

    @ApiModelProperty(value = "设备价格")
    private Integer devicePrice;

    @ApiModelProperty(value = "保修时间（实际用作：学期）")
    private String guarantee;

    @ApiModelProperty(value = "生产厂商（实际用作：检查人员）")
    private String deviceFactory;

    @ApiModelProperty(value = "设备状态（实际用作：检查状态）")
    private Integer deviceStatus;

    @ApiModelProperty(value = "实验室ID")
    private Integer labId;
}'''
    doc.add_paragraph(device_entity_code)
    
    doc.add_heading('（4）运行界面（截图）', level=4)
    doc.add_paragraph('图7 安全检查登记界面')
    doc.add_paragraph('图8 新增安全检查记录界面')
    doc.add_paragraph('【此处插入安全检查登记界面截图】')
    doc.add_paragraph('【此处插入新增安全检查记录界面截图】')
...
if __name__ == '__main__':
    output_file = create_doc_with_real_safety_check()
    print(f"技术开发文档生成完成！")
